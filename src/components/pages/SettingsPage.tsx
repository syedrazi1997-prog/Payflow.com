  ...
  async function handleCreateKey() {
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(error.message || 'Unable to create API key.');
      }

      // Supabase returns the JSONB object directly for this function.
      // Keep a small compatibility fallback for older function versions.
      const result = Array.isArray(data) ? data[0] : data;

      if (!result || result.success !== true || typeof result.key !== 'string') {
        throw new Error(
          result?.message ||
          'The API key was not created. Check that the Supabase migration has been applied.'
        );
      }
      ...
      window.alert(err instanceof Error ? err.message : 'Unable to create API key.');
    } finally {
